...
1. Read `QA_REPORT.md` — check for open issues assigned to your role and resolve them before new work
2. Read your own role file in `.agents/<role>.md`
...
|-------|-----------|----------------|
| Frontend | `.agents/frontend.md` | `src/components/`, `src/App.tsx`, `src/main.tsx`, `src/index.css` |
| Backend | `.agents/backend.md` | `src/lib/`, `src/hooks/` |
| Data Engineer | `.agents/data-engineer.md` | `src/data/`, `src/types/` |
| DevOps | `.agents/devops.md` | `package.json`, config files, `.env*` (exclusive) |
| QA/QC | `.agents/qa-qc.md` | `QA_REPORT.md` (primary), read-only on all code |
