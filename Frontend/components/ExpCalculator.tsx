...
import { calcExpProgress, calcExpSession, resolveBuffs } from '../../Backend/lib/expCalc'
import { formatExpLarge } from '../../Backend/lib/format'
        ...
        <Panel className="sticky-panel">
          ...
          {mode === 'session' ? (
            <>
              ...
              <ProgressBar value={sessionResult?.pctOfLevel30min ?? 0} tone="teal" />
          ...
          />
          <p className="fine-print">Session percent: {(sessionResult?.pctOfLevel30min ?? 0).toFixed(2)}%</p>
        </Panel>
      </aside>
    </div>
  )
